...
class WolfSheep(Model):
    ...
    def __init__(
        ...
        self.datacollector = DataCollector(
            {
                "Average Grass Growth": lambda x: x.__get_average_metric_for(GrassPatch, lambda g: g.progress),
                
                "Average Sheep Energy": lambda x: x.__get_average_metric_for(Sheep, lambda s: s.energy),
                "Max Sheep Energy": lambda x: x.__get_max_metric_for(Sheep, lambda s: s.energy),

                "Average Wolf Energy": lambda x: x.__get_average_metric_for(Wolf, lambda w: w.energy),
                "Max Wolf Energy": lambda x: x.__get_max_metric_for(Wolf, lambda w: w.energy),
        ...
        for i in range(self.sheep_initial_count):
            ...
            sheep = self.create_sheep(cell, self.random.randrange(1,100))
        ...
        for i in range(self.wolf_initial_count):
            ...
            wolf = self.create_wolf(cell, self.random.randrange(1,100))
        ...
        for x in range(self.grid.width):
            for y in range(self.grid.height):
                grass = GrassPatch(self.next_id(), self, self.random.randrange(0,100), self.grass_progress_per_step)
                self.add_agent(grass, (x, y))

    def create_sheep(self, pos: Coordinate, energy: float):
        return Sheep(self.next_id(), self, self.grid, pos, self.moore, energy, self.sheep_energy_step_expenditure, self.sheep_energy_gain_from_food, self.sheep_reproduction_chance, self.sheep_reproduction_chance)

    def create_wolf(self, pos: Coordinate, energy: float):
        return Wolf(self.next_id(), self, self.grid, pos, self.moore, energy, self.sheep_energy_step_expenditure, self.wolf_energy_gain_from_food,self.sheep_reproduction_chance, self.sheep_reproduction_chance)
    ...
    def step(self):
        ...
        # wolves: List[Wolf] = list(filter(lambda a: isinstance(a, Wolf), self.schedule.agents))
        # for wolf in wolves:
    ...
    def reproduce_animals(self):
        animals_to_reproduce: List[Animal] = filter(lambda a: isinstance(a, Animal), self.schedule.agents)
        ...
        for animal in animals_to_reproduce: 
            ...
            if (chance < animal.reproduction_chance):
                if(type(animal) is Sheep):
                    to_create = self.create_sheep(animal.pos, 10)
                if(type(animal) is Wolf):
                    to_create = self.create_wolf(animal.pos, 10)
                self.add_agent(to_create, animal.pos)
                animal.energy -= animal.reproduction_energy_cost

    def feed_wolves(self):
        wolves_to_feed : List[Wolf] = filter(lambda a: isinstance(a, Wolf), self.schedule.agents)
    ...
    def feed_sheeps(self):
        sheeps_to_feed : List[Sheep] = filter(lambda a: isinstance(a, Sheep), self.schedule.agents)
        ...
        for sheep in sheeps_to_feed:
            ...
            grass_in_cell: GrassPatch = list(filter(lambda a: isinstance(a, GrassPatch), cell_content))[0]
    ...
    def kill_animals(self):
        animals_to_kill: List[Animal] = filter(lambda a: isinstance(a, Animal), self.schedule.agents)
